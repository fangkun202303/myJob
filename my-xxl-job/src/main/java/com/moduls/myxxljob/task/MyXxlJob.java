...
public class MyXxlJob extends IJobHandler {
    ...
    public ReturnT<String> execute(String s) throws Exception {
        ...
        System.out.println(LocalTime.now().atDate(LocalDate.now()).format(DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss"))+"-->>> MyXxlJob执行任务");
        logger.info(LocalTime.now().atDate(LocalDate.now()).format(DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss"))+
                "执行我的服务了，方法com.moduls.myxxljob.task.MyXxlJob正在run");
        return SUCCESS;
    }
}
